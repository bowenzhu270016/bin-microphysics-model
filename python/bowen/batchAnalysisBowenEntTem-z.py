...
def plot_model_run():
    ...
    for i in range(len(namelist_files)):
        ...
        for j in range(len(runToDo[0])):
            ...
            try:
                ...
                if 'qi' in nc.variables.keys():
                    ...
                    ax.plot(Temperature-273.15, z, label=f"{runToDo[0][j]}")  # 使用参数组合作为标签
                nc.close()
        ...
        if ax.is_first_col():
            ...
            ax.set_ylabel('Height (m)', fontsize=10)  # 设置纵坐标标签
        else:
            # ax.set_yticks([])  # 隐藏刻度
            ax.set_ylabel('')  # 隐藏标签
        ...
        if ax.is_last_row():
            ax.set_xlabel(r'T ($^\circ$C)', fontsize=10)  # 设置横坐标标签
        else:
            # ax.set_xticks([])  # 隐藏刻度
            ax.set_xlabel('')  # 隐藏标签
